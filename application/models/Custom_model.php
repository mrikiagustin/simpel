...
class Custom_model  extends CI_Model  {
    ...
    function get_list()
    {
        $this->db->select("{$this->table_name}.id_permohonan ,{$this->table_name}.created_at , group_concat(distinct c.kode_contoh) kode_contoh ", false);
        $this->db->join('permohonan_detail b', "b.id_permohonan = {$this->table_name}.id_permohonan", 'left');
        $this->db->join('permohonan_detail_parameter c', "c.id_permohonan_detail = b.id_permohonan_detail", 'left');
        $this->db->join('parameter_pengujian d', "d.id_parameter_pengujian = c.id_parameter_pengujian", 'left');
        $this->db->group_by("{$this->table_name}.id_permohonan");

        if(count($this->id_laporan) > 0) $this->db->where_in("d.id_laporan",$this->id_laporan);
    ...
    function set_id_laporan($id_laporan)
    {
