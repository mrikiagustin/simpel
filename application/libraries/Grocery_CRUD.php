...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function set_ajax_list_queries($state_info = null)
	{
		...
		if (!empty($state_info->filter)) {

			$has_callbacks = !empty($this->callback_filter) ? true : false;



			if ($has_callbacks) {
				foreach ($state_info->filter as $key => $value) {
					if (isset($this->callback_filter[$key])) {
						call_user_func($this->callback_filter[$key], $this, $value);
					} else {
						echo "callback_filter doesnt exist : " . $key;
						die();
					}
				}
			} else {
				echo "u dont have any filter callbacks ";
				die();
			}
		}
	}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showEditForm($state_info)
	{
		...
		$data->is_ajax 			= $this->_is_ajax();
		$data->primary_key_value = $state_info->primary_key;
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $custom_button			= false;
	protected $custom_button_text		= '';
	protected $custom_button_link		= '';

	protected $custom_button2			= false;
	protected $custom_button2_text		= '';
	protected $custom_button2_link		= '';
	...
	protected $callback_filter			= array();
	...
	public function unset_save()
	{
		$this->unset_save = true;

		return $this;
	}

	public function set_custom_button($text, $link)
	{
		$this->custom_button = true;
		$this->custom_button_text = $text;
		$this->custom_button_link = $link;

		return $this;
	}

	public function set_custom_button2($text, $link)
	{
		$this->custom_button2 = true;
		$this->custom_button2_text = $text;
		$this->custom_button2_link = $link;
	...
	protected function _initialize_variables()
	{
		...
		$this->config->default_language 	= "indonesian"; //$ci->config->item('grocery_crud_default_language');
	...
	public function callback_filter($filter, $callback = null)
	{
		$this->callback_filter[$filter] = $callback;
