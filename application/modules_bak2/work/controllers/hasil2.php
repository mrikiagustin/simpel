...
class Hasil2 extends CI_Controller {
	...
	public function pdf($id, $download = 0, $header = 0,$tanggal = "")
	{
		...
		for ($i=0; $i < $count_content; $i++) {
			...
			$true_content[] = $this->load->view($view_name,array("data" => $data,"data2" => $data2,"ttd" => $ttd , "start"=>$start , "end"=>$end,"data_detail" => $data_detail,"tgl"=>$tanggal,"header" => 0),true);
		}
	...
	public function hasil_pengujian_kn($value = '', $primary_key = null)
  {
    ...
    $data = $this->db
                    ->where("nomor_contoh", $nomor)
					//->order_by("laporan , parameter_pengujian")
                    //->where("id_laporan", $laporan)
