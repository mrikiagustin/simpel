...
<table border="1" cellpadding="5" class="isi" style="width:100%">
  ...
  <?php $no = 1;$no2 = 1;foreach ($data->mdetail as $k => $v):
        $rowspan = count($v->mpengujian_paket) + count($v->mpengujian) == 0 ? 1 : count($v->mpengujian_paket) + count($v->mpengujian);
  ?>
	<?php if($no >= $start && $no <= $end):?>

  <tr>
    <td rowspan="<?php echo $rowspan?>"><?php echo $no2?> </td>
    ...
    <td><?php echo isset($v->mpengujian_paket[0]->caption) && $v->mpengujian_paket[0]->caption != '' ? $v->mpengujian_paket[0]->caption :  (isset($v->mpengujian_paket[0]->parameter_pengujian) ? $v->mpengujian_paket[0]->parameter_pengujian : "-")  ?></td>
    <td><?php echo isset($v->mpengujian_paket[0]->metode) ? $v->mpengujian_paket[0]->metode : "-"  ?></td>
    <td rowspan="<?php echo $rowspan?>"><?php echo $v->keterangan?></td>
  </tr>
	<?php endif;$no++;$no2++?>

  <?php for ($i=1; $i < count($v->mpengujian_paket); $i++) :?>
		<?php if($no >= $start && $no <= $end):?>
    <tr>
			<?php if($no == $start){
				//hitung dulu
				$zxczxc = count($v->mpengujian_paket) + count($v->mpengujian) - $i;
			?>
		    ...
		    <td rowspan="<?php echo $zxczxc?>"></td>
		    <td rowspan="<?php echo $zxczxc?>"></td>
				<td><?php echo isset($v->mpengujian_paket[$i]->caption) && $v->mpengujian_paket[$i]->caption != '' ? $v->mpengujian_paket[$i]->caption :  (isset($v->mpengujian_paket[$i]->parameter_pengujian) ? $v->mpengujian_paket[$i]->parameter_pengujian : "-")  ?></td>
	      <td><?php echo isset($v->mpengujian_paket[$i]->metode) ? $v->mpengujian_paket[$i]->metode : "-"  ?></td>
				<td rowspan="<?php echo $zxczxc?>"></td>
			<?php }else{?>
      <td><?php echo isset($v->mpengujian_paket[$i]->caption) && $v->mpengujian_paket[$i]->caption != '' ? $v->mpengujian_paket[$i]->caption :  (isset($v->mpengujian_paket[$i]->parameter_pengujian) ? $v->mpengujian_paket[$i]->parameter_pengujian : "-")  ?></td>
      <td><?php echo isset($v->mpengujian_paket[$i]->metode) ? $v->mpengujian_paket[$i]->metode : "-"  ?></td>
  ...
  <?php for ($i=0; $i < count($v->mpengujian); $i++) :?>
		<?php if($no >= $start && $no <= $end):?>
    <tr>
			<?php if($no == $start){
				//hitung dulu
