...
<table cellpadding="2" class="meta" border="1">
  ...
  <tr>
    ...
    <td>".(isset($v->mpengujian_paket[0]->biaya) ? "Rp. ".number_format($v->mpengujian_paket[0]->biaya,0,",",".") : "-")."</td>
  ...
  for ($i=1; $i < count($v->mpengujian_paket); $i++) :
    ...
    if($no >= $start && $no <= $end):
      if($no == $start){
				//hitung dulu
				$zxczxc = count($v->mpengujian_nonpaket) + count($v->mpengujian_paket) - $i;
        ...
        <tr>
          ...
          <td rowspan=\"".$zxczxc."\"></td>
          <td rowspan=\"".$zxczxc."\"></td>
          <td>".$no."-".(isset($v->mpengujian_paket[$i]->caption) && $v->mpengujian_paket[$i]->caption != '' ? $v->mpengujian_paket[$i]->caption :  (isset($v->mpengujian_paket[$i]->parameter_pengujian) ? $v->mpengujian_paket[$i]->parameter_pengujian : "-"))."</td>
          <td>".(isset($v->mpengujian_paket[$i]->biaya) ? "Rp. ".number_format($v->mpengujian_paket[$i]->biaya,0,",",".") : "-")."</td>
          <td rowspan=\"".$zxczxc."\"></td>
        </tr>";
      }else{
        $html .= "
    ...
    if($no >= $start && $no <= $end):
      if($no == $start){
        ...
        <tr>
          ...
          <td rowspan=\"".$zxczxc."\"></td>
          <td rowspan=\"".$zxczxc."\"></td>
          <td>".$no."-".(isset($v->mpengujian_nonpaket[$i]->caption) && $v->mpengujian_nonpaket[$i]->caption != '' ? $v->mpengujian_nonpaket[$i]->caption :  (isset($v->mpengujian_nonpaket[$i]->parameter_pengujian) ? $v->mpengujian_nonpaket[$i]->parameter_pengujian : "-"))."</td>
          <td>".(isset($v->mpengujian_nonpaket[$i]->biaya) ? "Rp. ".number_format($v->mpengujian_nonpaket[$i]->biaya,0,",",".") : "-")."</td>
          <td rowspan=\"".$zxczxc."\"></td>
        </tr>";
      }else{
        $html .= "
