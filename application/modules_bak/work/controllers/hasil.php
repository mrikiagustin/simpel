...
class Hasil extends CI_Controller {
	...
	function __construct(){
  	...
  	$this->load->model("permohonan_model");
		...
		public function word($id)
		{
			...
			$templateProcessor->setValue('tanggal', $this->tgl_indo(date("Y-m-d")));
			$templateProcessor->setValue('kode_dokumen', $kkode);

			$d = $data[0];
			$templateProcessor->setValue('contoh',  $d->varietas);
			$templateProcessor->setValue('tanggal_terima',  $this->tgl_indo($d->tanggal_masuk));
			...
			$templateProcessor->setValue('tanggal_selesai',  $this->tgl_indo($d->tanggal_pengambilan));
	...
	public function pdf($id, $download = 0, $header = 0)
	{
		...
		$content = $this->load->view("hasil_pdf",array("data" => $data,"data2" => $data2,"kode_dokumen" => $kkode),true);
		...
		for ($i=0; $i < $count_content; $i++) {
			...
			$true_content[] = $this->load->view($view_name,array("data" => $data,"data2" => $data2,"ttd" => $ttd , "start"=>$start , "end"=>$end,"data_detail" => $data_detail),true);
		}
            	...
            	'keterangan');
        			...
        			$crud->edit_fields('prim','print','nomor_contoh','laporan','nomor_seri','ceklis_1','ceklis_2','hasil_pengujian');
							...
							$crud->callback_edit_field('nomor_seri', function ($t = 0) {
								if($t == 0){
										return '<input type="text" value="( auto number )" readonly="true" name="nomor_seri" style="background:#eee;">';
								}
        				return '<input type="text" value="'.$t.'" readonly="true" name="nomor_seri" style="background:#eee;">';
        			});
  ...
  public function hasil_pengujian($value = '', $primary_key = null)
  {
    ...
    foreach ($data->result() as $key => $value) {
      ...
      $html .= "<td><input type='text' name='h[".$value->id_permohonan_detail_parameter."][keterangan]' value='".$value->keterangan_hasil."'></td>";
			$html .= "<td><select name='h[".$value->id_permohonan_detail_parameter."][standard]'>
											...
											<option value='3'> Terdeteksi </option>
											<option value='4'> Tidak Terdeteksi </option>
										</select></td>";
      $html .= "</tr>";
    }
	...
	public function _update_callback($post_array)
	{
				...
				if($is_update){
					$this->db->update("hasil",$dx,array("prim" => $post_array["prim"]));
				}else{
					$year = date("Y");
