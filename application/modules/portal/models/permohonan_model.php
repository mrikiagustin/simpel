...
class Permohonan_model  extends CI_Model
{
	...
	public function get($id, $additional_params = array())
	{
		...
		for ($i = 0; $i < count($main_data->mdetail); $i++) {
			...
			$main_data->mdetail[$i]->mpengujian = $this->db->get("pub_permohonan_detail_parameter mpengujian")->result();
			...
			$main_data->mdetail[$i]->mpengujian_nonpaket = $this->db->get("pub_permohonan_detail_parameter mpengujian")->result();
			...
			$main_data->mdetail[$i]->mpengujian_paket = $this->db->get("pub_permohonan_detail_parameter mpengujian")->result();
