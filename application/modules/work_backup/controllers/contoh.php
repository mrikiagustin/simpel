		...
		public function _update_callback($post_array)
		{
					...
					foreach ($post_array['kode_contoh'] as $key => $value) {
						...
						//var_dump($v);die();
							...
							//echo " <br> ". $paten[$key][$k];die();
							//print_r($paten[$key][$k]);echo " - ".$k." <br> ";
							foreach ($v as $z => $x) {
					...
					foreach ($post_array['kode_contoh'] as $key => $value) {
						...
						foreach ($value as $k => $v) {
							foreach ($v as $z => $x) {
								...
								if($atasnya == "") $atasnya = $x;
								if(substr($x,0,3) == "000" && substr($x,4,2) == substr($atasnya,4,2)){							
									//$atasnya = $x;
								}else{
									$atasnya = $x;
								}
								//echo $atasnya."<br>";
								$this->db->update("permohonan_detail_parameter",array("kode_contoh" => $atasnya),array("id_permohonan_detail_parameter" => $z));
